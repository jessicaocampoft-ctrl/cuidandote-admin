...
- Funciones relacionadas: 63
- IDs relacionados: 23
- Acciones API relacionadas: 3
- Estados canónicos detectados: 3
...
### setupOperationsModuleUI — línea 6801
...
### renderProfessionalAgenda — línea 7209
...
### initFunctionalModules2026 — línea 7531
...
### renderCentroAcciones — línea 7761
...
```javascript
function pagoBadge(id) {
  const c      = allData.citas.find(x => x.id === id);
  const future = c && normDate(c.fecha) > today();
  if (future) return `<span class="pago-btn" style="color:var(--muted);border-color:var(--border);cursor:default;opacity:.6">📅 Próxima</span>`;
  const metodo = getMetodoPago(id);
  const paid   = !!(metodo) || kvGet('pago_'+id) === '1';
  const label  = metodo ? '✓ ' + metodo : (paid ? '✓ Pagada' : '⏳ Pago pend.');
  return `<button type="button" class="pago-btn ${paid?'pagada':'pendiente'}" onclick="event.stopPropagation();abrirModalPago('${id}')" title="Clic para registrar método de pago">${label}</button>`;
}
```

### esCobrada — línea 8137
...
### smartIsPaid — línea 8936

```javascript
function smartIsPaid(c) {
  if (!c) return false;
  return !!c.pago || kvGet('pago_' + c.id) === '1' || (typeof isPagada === 'function' && isPagada(c.id));
}
```

### renderSmartBriefing — línea 9026
...
function _segCard(p) {
  ...
  return `<div class="seg-card ${reagendado?'reagendado':''}">
    ...
    <div style="display:flex;gap:6px;flex-wrap:wrap;flex-shrink:0">
      ...
      ` : `
        ${p.semana==='sem3' && wa3 ? `<a href="${wa3}" target="_blank" class="btn btn-sm btn-purple" onclick="segMarkWa('${p.nombre.replace(/'/g,"\\'")}
```

### _segCardReadap — línea 12446
...
function _segCardReadap(p) {
  ...
  return `<div class="seg-card ${reagendado?'reagendado':''}" style="border-left:3px solid #0369a1">
    ...
    <div style="display:flex;gap:6px;flex-wrap:wrap;flex-shrink:0">
      ${reagendado ? `
        <button class="btn btn-ghost btn-sm" onclick="segToggleR('${p.nombre.replace(/'/g,"\\'")}
```

### exportarSeguimientoCSV — línea 12558
...
### _copyGestionAcciones — línea 13843
...
### _copyGestionAsesorText — línea 13894
...
### abrirCopiarListaGestion — línea 14016
...
### _checkCobrosPendientes — línea 16967
...
### renderCitasResumen — línea 17141
...
### marcarComisionPagada — línea 19072
...
### desmarcarComisionPagada — línea 19077
...
### renderRecuperaciones — línea 19670
