# Inventario para modularización de configuración y API

Archivo: `index.html`

## Configuración y estado encontrados
...
## Funciones conocidas de comunicación
...
## Funciones que contienen fetch(

### loadAdminKV — línea 6150
...
setInterval(() => {
  if (TOKEN && Date.now() - _lastActivity > _INACTIVITY_MS) {
    toast('Sesión cerrada por inactividad (30 min).', 'warn');
    setTimeout(logout, 1500);
  }
}, 60_000);

// ── VERIFICAR SESIÓN AL VOLVER A LA PESTAÑA ──
// Cubre el caso de suspensión del equipo donde JS se pausa y los timers no disparan
...
### saveProfessionalForm — línea 6602
...
### setupOperationsModuleUI — línea 6752
...
### changeProfessionalPassword — línea 7189
...
### loadProfessionalAgenda — línea 7210
...
### markProfessionalAttended — línea 7269
...
### sendProfessionalIssue — línea 7286
...
### _runUrlRepairIfRequested — línea 7435
...
### loadKPIHistoryFromServer — línea 7811
...
### loadAutomationCenter — línea 8022

```javascript
async function loadAutomationCenter() {
  const title=document.getElementById('autoStatusTitle'); if(!title)return;
  title.textContent='Consultando estado...';
  try {
    const [status,queue]=await Promise.all([
      fetch(`${APPS_SCRIPT_URL}?action=automationStatus&token=${encodeURIComponent(TOKEN)}`).then(r=>r.json()),
      fetch(`${APPS_SCRIPT_URL}?action=automationQueue&status=pending&token=${encodeURIComponent(TOKEN)}`).then(r=>r.json())
    ]);
    if(!status.ok)throw new Error(status.error||'Actualiza la implementación de Apps Script');
    document.getElementById('autoStatusDot').classList.toggle('on',!!status.active);
    title.textContent=status.active?'Automatizaciones activas':'Programación pendiente de activar';
    document.getElementById('autoStatusSub').textContent=status.active?'El sistema trabaja a las 7am, 10pm y los lunes a las 8am.':'Pulsa “Activar programación” una vez después de actualizar Apps Script.';
    document.querySelectorAll('[data-auto-key]').forEach(i=>i.checked=!!status.config[i.dataset.autoKey]);
    renderAutomationQueue(queue.items||[]);
    renderAutomationLogs(status.logs||[]);
  } catch(e) {
    title.textContent='El servidor aún no tiene el motor actualizado';
    document.getElementById('autoStatusSub').textContent='Copia y vuelve a implementar google-apps-script.js para activar estas funciones.';
    document.getElementById('autoStatusDot').classList.remove('on');
  }
}
```

### saveAutomationSettings — línea 8044

```javascript
async function saveAutomationSettings() {
  const config={};document.querySelectorAll('[data-auto-key]').forEach(i=>config[i.dataset.autoKey]=i.checked);
  try {
    const d=await fetch(`${APPS_SCRIPT_URL}?action=automationSave&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(config))}`).then(r=>r.json());
    if(!d.ok)throw new Error(d.error);toast('Configuración de automatizaciones guardada');
  } catch(e){toast('No se pudo guardar: actualiza Apps Script','err')}
}
```

### setupAutomations — línea 8052

```javascript
async function setupAutomations() {
  try {
    toast('Activando tareas programadas...');
    const d=await fetch(`${APPS_SCRIPT_URL}?action=automationSetup&token=${encodeURIComponent(TOKEN)}`).then(r=>r.json());
    if(!d.ok)throw new Error(d.error);toast('Automatizaciones activadas');loadAutomationCenter();
  } catch(e){toast('No se pudieron activar: '+(e.message||'revisa permisos'),'err')}
}
```

### runAutomationJob — línea 8060

```javascript
async function runAutomationJob(job) {
  try {
    toast('Ejecutando automatización...');
    const d=await fetch(`${APPS_SCRIPT_URL}?action=automationRun&job=${encodeURIComponent(job)}&token=${encodeURIComponent(TOKEN)}`).then(r=>r.json());
    if(!d.ok)throw new Error(d.error);toast('Automatización completada');loadAutomationCenter();
  } catch(e){toast('Error al ejecutar: '+(e.message||'revisa el servidor'),'err')}
}
```

### markAutomationMessage — línea 8080

```javascript
async function markAutomationMessage(id) {
  setTimeout(async()=>{try{await fetch(`${APPS_SCRIPT_URL}?action=automationQueueDone&id=${encodeURIComponent(id)}&token=${encodeURIComponent(TOKEN)}`).then(r=>r.json());loadAutomationCenter()}catch(e){}},700);
}
```

### confirmarPago — línea 8291

```javascript
async function confirmarPago(metodo) {
  if (!_pagoIdActivo) return;
  const id = _pagoIdActivo;
  cerrarModalPago();
  try {
    const r = await fetch(`${APPS_SCRIPT_URL}?action=updatePago&token=${encodeURIComponent(TOKEN)}&id=${encodeURIComponent(id)}&metodo=${encodeURIComponent(metodo)}`).then(x => x.json());
    if (r.ok) {
      const c = allData.citas.find(x => x.id === id);
      if (c) c.pago = metodo;
      if (metodo) kvSet('pago_'+id, '1');
      else kvRemove('pago_'+id);
      if (c) logChange(metodo ? 'Pago registrado' : 'Pago pendiente', `${c.nombre} · ${c.fecha} ${c.hora} · ${metodo || 'sin método'}`);
      toast(metodo ? 'Pago registrado: ' + metodo : 'Marcado como pendiente', 'ok');
      renderAgenda(); initDashboard(); renderFinanzas();
    } else toast('Error al guardar: ' + (r.error || ''), 'err');
  } catch(e) { toast('Error de conexión', 'err'); }
}
```

### submitAdminBookingMulti — línea 8607
...
async function reload() {
  try {
    const r = await fetch(APPS_SCRIPT_URL + '?action=adminData&token=' + encodeURIComponent(TOKEN));
    if (!r.ok) { toast('Error del servidor (' + r.status + '). Recarga la página.', 'err'); return; }
    const d = await r.json();
    if (d.ok) {
      allData = d;
      await loadTeamData();
    }
    else if (d.error === 'Sin permiso') { toast('Sesión expirada. Volviendo al login...', 'warn'); setTimeout(logout, 1500); }
  } catch(e) {
    toast('Sin conexión. Verifica tu internet.', 'err');
  }
}
```

### guardarNotaAdmin — línea 9754
...
### confirmarCancelacion — línea 9890
...
### submitAdminBooking — línea 10820
...
### exportarHistorialPaciente — línea 11278
...
### cargarRecordatorios — línea 12169
...
### enviarEmailsRecordatorio — línea 12284
...
### limpiarCitasSinHora — línea 12665
...
### exportarSeguimientoCSV — línea 12690
...
### limpiarDuplicadosGAS — línea 12887
...
// ── Interceptor WA en desktop: muestra modal para copiar en vez de abrir wa.me ──
(function initWACopyInterceptor() {
  const isMobile = /Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
  if (isMobile) return; // en celular, el link funciona directo con la app nativa
  ...
  document.addEventListener('click', function(e) {
    ...
    if (a.dataset.noIntercept) return; // skip botones del propio modal
    // Hay un link de WhatsApp: interceptar en desktop
    e.preventDefault();
    try {
      const url    = new URL(href.startsWith('http') ? href : 'https:' + href);
      const rawMsg = url.searchParams.get('text') || '';
      ...
      // "Abrir WhatsApp Web" abre el chat directo sin text= para evitar la corrupcion de emojis en el redirect
      const chatUrl = 'https://web.whatsapp.com/send?phone=' + phone;
      document.getElementById('waCopyText').textContent = msg;
      document.getElementById('waOpenLink').href        = chatUrl;
    } catch(_) {
      document.getElementById('waCopyText').textContent = '';
      document.getElementById('waOpenLink').href        = 'https://web.whatsapp.com';
    }
    document.getElementById('waCopyModal').style.display = 'flex';
  }); // bubble phase: los onclick inline (markWaSent, etc.) disparan primero, luego prevenimos la navegacion
})();
...
function toggleHelpBanner(el) {
  el.classList.toggle('open');
  const id = el.getAttribute('data-help-id');
  if (id) {
    try {
      localStorage.setItem('helpBanner_' + id, el.classList.contains('open') ? '1' : '0');
    } catch(e) {}
  }
}

function restoreHelpBanners() {
  document.querySelectorAll('.help-banner[data-help-id]').forEach(el => {
    const id = el.getAttribute('data-help-id');
    try {
      const saved = localStorage.getItem('helpBanner_' + id);
      // Abiertos por defecto la primera vez; cerrados si el usuario los cerró
      if (saved === '0') el.classList.remove('open');
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(actualizarContadorLeads, 500);
  setTimeout(restoreHelpBanners, 300);
});
...
// ══════════════════════════════════════════
const DISP_SLOTS_ORDER = ['07:00','08:00','09:00','10:00','11:00','13:00','14:00','15:00','16:00','17:00','18:00','19:00'];

function abrirModalDisp() {
  const modal = document.getElementById('modalDisp');
  if (!modal) return;
  // Fecha de hoy por defecto
  const fechaInp = document.getElementById('dispFecha');
  if (fechaInp && !fechaInp.value) {
    fechaInp.value = today();
...
function buscarDisponibilidad() {
  ...
  _dispDebounce = setTimeout(_buscarDisp, 300);
}

async function _buscarDisp() {
  const fecha    = document.getElementById('dispFecha')?.value;
  const servicio = document.getElementById('dispServicio')?.value;
  const slotsEl  = document.getElementById('dispSlots');
  const copyWrap = document.getElementById('dispCopyWrap');
  if (!fecha || !slotsEl) return;

  slotsEl.innerHTML = '<span style="color:var(--muted)">Consultando...</span>';
  if (copyWrap) copyWrap.style.display = 'none';

  try {
    const url = `${APPS_SCRIPT_URL}?action=availability&date=${encodeURIComponent(fecha)}&service=${encodeURIComponent(servicio||'')}`;
    const res  = await fetch(url).then(r => r.json());
    if (!res.ok) throw new Error(res.error || 'Error');

    const slots = res.slots || {};
    const libres = DISP_SLOTS_ORDER.filter(s => slots[s]);
    const html = DISP_SLOTS_ORDER.map(s => {
      const libre = !!slots[s];
      const h = parseInt(s.split(':')[0]);
      const label = h < 12 ? s + ' AM' : (h === 12 ? '12:00 PM' : (h-12) + ':00 PM');
      return `<div class="disp-slot ${libre ? 'libre' : 'ocupado'}">${libre ? '✓ ' : ''}${label}</div>`;
    }).join('');

    slotsEl.innerHTML = libres.length
      ? `<div class="disp-slots">${html}</div>`
      : `<div class="disp-slots">${html}</div><p style="text-align:center;color:var(--muted);font-size:.85rem;margin:0">Sin horarios disponibles este día</p>`;

    if (copyWrap) copyWrap.style.display = libres.length ? 'block' : 'none';
  } catch(e) {
    slotsEl.innerHTML = '<span style="color:#ef4444;font-size:.85rem">Error al consultar. Revisa la conexión.</span>';
  }
}

function copiarDisponibilidadWA() {
  const fecha    = document.getElementById('dispFecha')?.value;
  const servicio = document.getElementById('dispServicio')?.value || '';
  if (!fecha) return;
  ...
  const DIAS  = ['domingo','lunes','martes','miércoles','jueves','viernes','sábado'];
  const MESES = ['enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre'];
  const dp = fecha.split('-');
  const fObj = new Date(+dp[0], +dp[1]-1, +dp[2]);
  const fechaLegible = DIAS[fObj.getDay()] + ' ' + +dp[2] + ' de ' + MESES[+dp[1]-1];

  // Recoger slots libres del DOM
  const libres = [];
  document.querySelectorAll('#dispSlots .disp-slot.libre').forEach(el => {
    libres.push(el.textContent.trim().replace('✓ ', ''));
  });
  if (!libres.length) return;

  const msg =
    `📅 *Horarios disponibles — ${fechaLegible}*\n` +
    `🩺 Servicio: ${servicio}\n\n` +
    libres.map(h => `✅ ${h}`).join('\n') +
    '\n\n¿Cuál te queda mejor? 😊';

  navigator.clipboard.writeText(msg).then(() => {
    const msgEl = document.getElementById('dispCopyMsg');
...
document.addEventListener('DOMContentLoaded', gCargarGuardados);
...
### generarLinkPasaporte — línea 18670
...
### guardarProgresoPasaporte — línea 18783
...
### regenerarTokenPasaporte — línea 18884
...
### desactivarPasaporte — línea 18894
...
### reactivarPasaporte — línea 18904
...
## Referencias directas a Apps Script

Total de referencias: 79.

### Referencia 1 — línea 6143
...
### Referencia 3 — línea 6188

```javascript
6186:   _kvDirty = {};
6187:   try {
6188:     await fetch(`${APPS_SCRIPT_URL}?action=setAdminKV&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(batch))}`);
6189:   } catch(e) {
6190:     Object.assign(_kvDirty, batch);
```

### Referencia 4 — línea 6200
...
### Referencia 5 — línea 6280

```javascript
6278:   if (!TOKEN) return teamData;
6279:   try {
6280:     const d = await fetch(`${APPS_SCRIPT_URL}?action=teamData&token=${encodeURIComponent(TOKEN)}`).then(r => r.json());
6281:     if (d.ok) {
6282:       teamData = {
```

### Referencia 6 — línea 6614

```javascript
6612:     tarifasJSON: '{}'
6613:   };
6614:   const d = await fetch(`${APPS_SCRIPT_URL}?action=saveProfessional&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(payload))}`).then(r => r.json());
6615:   if (!d.ok) { toast(d.error || 'No se pudo guardar', 'err'); return; }
6616:   closeModal('modalProfesional');
```

### Referencia 7 — línea 6625

```javascript
6623: async function resetProPassword(id) {
6624:   if (!confirm('¿Restablecer la contraseña de este usuario?')) return;
6625:   const d = await fetch(`${APPS_SCRIPT_URL}?action=resetProfessionalPassword&token=${encodeURIComponent(TOKEN)}&id=${encodeURIComponent(id)}`).then(r => r.json());
6626:   if (d.ok) showTemporaryPassword('Nueva contraseña temporal', d.tempPassword);
6627:   else toast(d.error || 'No se pudo restablecer', 'err');
```

### Referencia 8 — línea 6631

```javascript
6629: 
6630: async function togglePro(id, estado) {
6631:   const d = await fetch(`${APPS_SCRIPT_URL}?action=toggleProfessional&token=${encodeURIComponent(TOKEN)}&id=${encodeURIComponent(id)}&estado=${encodeURIComponent(estado)}`).then(r => r.json());
6632:   if (d.ok) { await loadTeamData(); renderEquipo(); toast('Estado actualizado'); }
6633:   else toast(d.error || 'No se pudo actualizar', 'err');
```

### Referencia 9 — línea 6639

```javascript
6637:   const label = nombre || 'este fisioterapeuta';
6638:   if (!confirm(`¿Eliminar a ${label} de la lista de fisioterapeutas?\n\nNo podrá ingresar al portal. El historial interno se conserva para auditoría.`)) return;
6639:   const d = await fetch(`${APPS_SCRIPT_URL}?action=deleteProfessional&token=${encodeURIComponent(TOKEN)}&id=${encodeURIComponent(id)}`).then(r => r.json());
6640:   if (d.ok) {
6641:     await loadTeamData();
```

### Referencia 10 — línea 6702

```javascript
6700:     override:document.getElementById('assignOverride').value
6701:   });
6702:   const d = await fetch(`${APPS_SCRIPT_URL}?${params.toString()}`).then(r => r.json());
6703:   if (d.ok) {
6704:     await loadTeamData();
```

### Referencia 11 — línea 6724

```javascript
6722:     excepcion:document.getElementById('assignExcepcion').value
6723:   });
6724:   const d = await fetch(`${APPS_SCRIPT_URL}?${params.toString()}`).then(r => r.json());
6725:   if (d.ok) {
6726:     closeModal('modalAsignarPro');
```

### Referencia 12 — línea 6736

```javascript
6734: async function markPayablePaid(id) {
6735:   if (!confirm('¿Marcar esta cuenta como pagada?')) return;
6736:   const d = await fetch(`${APPS_SCRIPT_URL}?action=markPayablePaid&token=${encodeURIComponent(TOKEN)}&id=${encodeURIComponent(id)}`).then(r => r.json());
6737:   if (d.ok) { await loadTeamData(); renderEquipo(); toast('Cuenta marcada como pagada'); }
6738:   else toast(d.error || 'No se pudo actualizar', 'err');
```

### Referencia 13 — línea 6744

```javascript
6742:   if (!TOKEN) return operationsData;
6743:   try {
6744:     const d = await fetch(`${APPS_SCRIPT_URL}?action=operationsData&token=${encodeURIComponent(TOKEN)}`).then(r => r.json());
6745:     if (d.ok) operationsData = d;
6746:   } catch(e) {
```

### Referencia 14 — línea 6753

```javascript
6751: 
6752: async function setupOperationsModuleUI() {
6753:   const d = await fetch(`${APPS_SCRIPT_URL}?action=setupOperationsModule&token=${encodeURIComponent(TOKEN)}`).then(r => r.json());
6754:   if (d.ok) {
6755:     await loadOperationsData();
```

### Referencia 15 — línea 6958

```javascript
6956:     };
6957: 
6958:     const d = await fetchJsonWithTimeout(APPS_SCRIPT_URL, {
6959:       method: 'POST',
6960:       body: JSON.stringify({ action: 'savePayment', token: TOKEN, data: payload })
```

### Referencia 16 — línea 6967

```javascript
6965:     if (mode === 'approve') {
6966:       const obs = observaciones || 'Pago confirmado desde registro de comprobante';
6967:       const verifyUrl = APPS_SCRIPT_URL
6968:         + '?action=verifyPayment&token=' + encodeURIComponent(TOKEN)
6969:         + '&id=' + encodeURIComponent(d.id)
```

### Referencia 17 — línea 7015

```javascript
7013:   if (!confirm(`¿Confirmas ${label}?`)) return;
7014:   const obs = prompt('Observación opcional para auditoría:', '') || '';
7015:   const d = await fetch(`${APPS_SCRIPT_URL}?action=verifyPayment&token=${encodeURIComponent(TOKEN)}&id=${encodeURIComponent(id)}&estado=${encodeURIComponent(status)}&observaciones=${encodeURIComponent(obs)}`).then(r => r.json());
7016:   if (d.ok) {
7017:     toast(status === 'Aprobado' ? 'Pago aprobado y cita autorizada' : 'Pago actualizado');
```

### Referencia 18 — línea 7163
...
### Referencia 20 — línea 7212

```javascript
7210: async function loadProfessionalAgenda() {
7211:   if (!PROFESSIONAL_TOKEN) return openProfessionalLoginMode();
7212:   const d = await fetch(`${APPS_SCRIPT_URL}?action=professionalAgenda&token=${encodeURIComponent(PROFESSIONAL_TOKEN)}`).then(r => r.json());
7213:   if (!d.ok) {
7214:     sessionStorage.removeItem('professionalToken');
```

### Referencia 21 — línea 7271

```javascript
7269: async function markProfessionalAttended(citaId) {
7270:   if (!confirm('¿Confirmas que esta sesión ya fue atendida? Esta acción solo la puede revertir administración.')) return;
7271:   const d = await fetch(APPS_SCRIPT_URL, {
7272:     method:'POST',
7273:     body:JSON.stringify({action:'professionalMarkAttended', token:PROFESSIONAL_TOKEN, citaId})
```

### Referencia 22 — línea 7287

```javascript
7285: 
7286: async function sendProfessionalIssue() {
7287:   const d = await fetch(APPS_SCRIPT_URL, {
7288:     method:'POST',
7289:     body:JSON.stringify({
```

### Referencia 23 — línea 7332

```javascript
7330:   btn.textContent = 'Verificando...'; btn.disabled = true;
7331:   try {
7332:     const d = await fetchJsonWithTimeout(APPS_SCRIPT_URL, {
7333:       method: 'POST',
7334:       body: JSON.stringify({action: 'adminLogin', user, password: pw})
```

### Referencia 24 — línea 7395

```javascript
7393:   if (document.visibilityState !== 'visible' || !TOKEN) return;
7394:   try {
7395:     const r = await fetch(`${APPS_SCRIPT_URL}?action=ping&token=${encodeURIComponent(TOKEN)}`).then(x => x.json());
7396:     if (!r.ok) {
7397:       toast('Sesión expirada. Volviendo al login...', 'warn');
```

### Referencia 25 — línea 7415

```javascript
7413:     const btn = document.getElementById('loginBtn');
7414:     try {
7415:       const r = await fetch(APPS_SCRIPT_URL + '?action=adminData&token=' + encodeURIComponent(TOKEN));
7416:       const d = await r.json();
7417:       if (d.ok) {
```

### Referencia 26 — línea 7443

```javascript
7441:   if (!nombre || !keepFecha || !keepHora) return;
7442:   try {
7443:     const url = `${APPS_SCRIPT_URL}?action=repairRescheduledDuplicate&token=${encodeURIComponent(TOKEN)}&nombre=${encodeURIComponent(nombre)}&keepFecha=${encodeURIComponent(keepFecha)}&keepHora=${encodeURIComponent(keepHora)}`;
7444:     const result = await fetch(url).then(r => r.json());
7445:     document.body.dataset.repairResult = JSON.stringify(result);
```

### Referencia 27 — línea 7554

```javascript
7552:   btn.textContent = 'Guardando...'; btn.disabled = true;
7553:   try {
7554:     const r = await fetch(APPS_SCRIPT_URL, {
7555:       method: 'POST',
7556:       body: JSON.stringify({action: 'changePassword', token: TOKEN, currentPassword: actual, newPassword: nueva})
```

### Referencia 28 — línea 7813

```javascript
7811: async function loadKPIHistoryFromServer() {
7812:   try {
7813:     const d=await fetch(`${APPS_SCRIPT_URL}?action=getKPIHistory&token=${encodeURIComponent(TOKEN)}`).then(r=>r.json());
7814:     if(d.ok){_kpiServerHistory={};(d.items||[]).forEach(x=>_kpiServerHistory[x.month]=x);}
7815:   } catch(e) {}
```

### Referencia 29 — línea 7967

```javascript
7965: async function _syncWaitList() {
7966:   try {
7967:     const d = await fetch(`${APPS_SCRIPT_URL}?action=getWaitlist&token=${encodeURIComponent(TOKEN)}`).then(r=>r.json());
7968:     if (d.ok) { _saveWaitList(d.items || []); _waitLoaded = true; return true; }
7969:   } catch(e) {}
```

### Referencia 30 — línea 7981

```javascript
7979:   const item = {id:'w'+Date.now(),nombre,telefono,servicio,preferencia,creado:new Date().toISOString()};
7980:   try {
7981:     const d = await fetch(`${APPS_SCRIPT_URL}?action=addWaitlist&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(item))}`).then(r=>r.json());
7982:     if (!d.ok) throw new Error(d.error||'No se pudo sincronizar');
7983:     item.id = d.id || item.id;
```

### Referencia 31 — línea 7991

```javascript
7989: 
7990: async function removeWaitPatient(id) {
7991:   try { await fetch(`${APPS_SCRIPT_URL}?action=removeWaitlist&token=${encodeURIComponent(TOKEN)}&id=${encodeURIComponent(id)}`).then(r=>r.json()); } catch(e) {}
7992:   _saveWaitList(_getWaitList().filter(p => p.id !== id));
7993:   renderWaitList(); toast('Paciente retirado de la lista');
```

### Referencia 32 — línea 8027

```javascript
8025:   try {
8026:     const [status,queue]=await Promise.all([
8027:       fetch(`${APPS_SCRIPT_URL}?action=automationStatus&token=${encodeURIComponent(TOKEN)}`).then(r=>r.json()),
8028:       fetch(`${APPS_SCRIPT_URL}?action=automationQueue&status=pending&token=${encodeURIComponent(TOKEN)}`).then(r=>r.json())
8029:     ]);
```

### Referencia 33 — línea 8028
...
8028:       fetch(`${APPS_SCRIPT_URL}?action=automationQueue&status=pending&token=${encodeURIComponent(TOKEN)}`).then(r=>r.json())
8029:     ]);
8030:     if(!status.ok)throw new Error(status.error||'Actualiza la implementación de Apps Script');
```

### Referencia 34 — línea 8047

```javascript
8045:   const config={};document.querySelectorAll('[data-auto-key]').forEach(i=>config[i.dataset.autoKey]=i.checked);
8046:   try {
8047:     const d=await fetch(`${APPS_SCRIPT_URL}?action=automationSave&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(config))}`).then(r=>r.json());
8048:     if(!d.ok)throw new Error(d.error);toast('Configuración de automatizaciones guardada');
8049:   } catch(e){toast('No se pudo guardar: actualiza Apps Script','err')}
```

### Referencia 35 — línea 8055

```javascript
8053:   try {
8054:     toast('Activando tareas programadas...');
8055:     const d=await fetch(`${APPS_SCRIPT_URL}?action=automationSetup&token=${encodeURIComponent(TOKEN)}`).then(r=>r.json());
8056:     if(!d.ok)throw new Error(d.error);toast('Automatizaciones activadas');loadAutomationCenter();
8057:   } catch(e){toast('No se pudieron activar: '+(e.message||'revisa permisos'),'err')}
```

### Referencia 36 — línea 8063

```javascript
8061:   try {
8062:     toast('Ejecutando automatización...');
8063:     const d=await fetch(`${APPS_SCRIPT_URL}?action=automationRun&job=${encodeURIComponent(job)}&token=${encodeURIComponent(TOKEN)}`).then(r=>r.json());
8064:     if(!d.ok)throw new Error(d.error);toast('Automatización completada');loadAutomationCenter();
8065:   } catch(e){toast('Error al ejecutar: '+(e.message||'revisa el servidor'),'err')}
```

### Referencia 37 — línea 8081

```javascript
8079: 
8080: async function markAutomationMessage(id) {
8081:   setTimeout(async()=>{try{await fetch(`${APPS_SCRIPT_URL}?action=automationQueueDone&id=${encodeURIComponent(id)}&token=${encodeURIComponent(TOKEN)}`).then(r=>r.json());loadAutomationCenter()}catch(e){}},700);
8082: }
8083: 
```

### Referencia 38 — línea 8296

```javascript
8294:   cerrarModalPago();
8295:   try {
8296:     const r = await fetch(`${APPS_SCRIPT_URL}?action=updatePago&token=${encodeURIComponent(TOKEN)}&id=${encodeURIComponent(id)}&metodo=${encodeURIComponent(metodo)}`).then(x => x.json());
8297:     if (r.ok) {
8298:       const c = allData.citas.find(x => x.id === id);
```

### Referencia 39 — línea 8703

```javascript
8701:     const data = { ...baseData, date: fechas[i].date, time: fechas[i].time };
8702:     try {
8703:       const r = await fetch(`${APPS_SCRIPT_URL}?action=adminBook&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(data))}`);
8704:       const d = await r.json();
8705:       if (d.ok) creadas++; else errores++;
```

### Referencia 40 — línea 8714

```javascript
8712:       document.getElementById('ncSubmitLabel').textContent = `Creando turno 2/${fechas.length}...`;
8713:       try {
8714:         const r2 = await fetch(`${APPS_SCRIPT_URL}?action=adminBook&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(data2))}`);
8715:         const d2 = await r2.json();
8716:         if (d2.ok) creadas++; else errores++;
```

### Referencia 41 — línea 8755

```javascript
8753:   }));
8754:   try {
8755:     const r = await fetch(`${APPS_SCRIPT_URL}?action=editBooking&token=${encodeURIComponent(TOKEN)}&data=${data}`);
8756:     const d = await r.json();
8757:     if (d.ok) {
```

### Referencia 42 — línea 9002

```javascript
9000: async function reload() {
9001:   try {
9002:     const r = await fetch(APPS_SCRIPT_URL + '?action=adminData&token=' + encodeURIComponent(TOKEN));
9003:     if (!r.ok) { toast('Error del servidor (' + r.status + '). Recarga la página.', 'err'); return; }
9004:     const d = await r.json();
```

### Referencia 43 — línea 9760

```javascript
9758:   try {
9759:     const data = encodeURIComponent(JSON.stringify({id, notaAdmin: nota}));
9760:     const r = await fetch(`${APPS_SCRIPT_URL}?action=editBooking&token=${encodeURIComponent(TOKEN)}&data=${data}`);
9761:     const d = await r.json();
9762:     if (d.ok) {
```

### Referencia 44 — línea 9772

```javascript
9770: async function changeStatus(id, status) {
9771:   try {
9772:     const r = await fetch(`${APPS_SCRIPT_URL}?action=updateStatus&token=${encodeURIComponent(TOKEN)}&id=${id}&status=${status}`);
9773:     const d = await r.json();
9774:     if (d.ok) {
```

### Referencia 45 — línea 9901

```javascript
9899: 
9900:   try {
9901:     const r = await fetch(`${APPS_SCRIPT_URL}?action=cancelBooking&token=${encodeURIComponent(TOKEN)}&id=${id}`);
9902:     const d = await r.json();
9903:     if (d.ok) {
```

### Referencia 46 — línea 10046

```javascript
10044:   try {
10045:     const data = encodeURIComponent(JSON.stringify({id, servicio, modalidad, fecha, hora, precio, notas}));
10046:     const r = await fetch(`${APPS_SCRIPT_URL}?action=editBooking&token=${encodeURIComponent(TOKEN)}&data=${data}`);
10047:     const d = await r.json();
10048:     if (d.ok) {
```

### Referencia 47 — línea 10095

```javascript
10093:   if (start >= end) { toast('La hora de fin debe ser mayor al inicio','err'); return; }
10094:   try {
10095:     const r = await fetch(`${APPS_SCRIPT_URL}?action=block&token=${encodeURIComponent(TOKEN)}&date=${date}&startTime=${start}&endTime=${end}&reason=${encodeURIComponent(reason||'Bloqueado')}`);
10096:     const d = await r.json();
10097:     if (d.ok) {
```

### Referencia 48 — línea 10109

```javascript
10107: async function doUnblock(bid, date, startTime) {
10108:   try {
10109:     const r = await fetch(`${APPS_SCRIPT_URL}?action=unblock&token=${encodeURIComponent(TOKEN)}&bid=${encodeURIComponent(bid)}&date=${date}&startTime=${startTime}`);
10110:     const d = await r.json();
10111:     if (d.ok) {
```

### Referencia 49 — línea 10669

```javascript
10667: 
10668:   try {
10669:     const r = await fetch(`${APPS_SCRIPT_URL}?action=crearEvento&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(data))}`);
10670:     const d = await r.json();
10671:     if (!d.ok) { toast('Error: ' + (d.error||''), 'err'); return; }
```

### Referencia 50 — línea 10699

```javascript
10697:   if (!confirm('¿Eliminar este evento?')) return;
10698:   try {
10699:     const r = await fetch(`${APPS_SCRIPT_URL}?action=eliminarEvento&token=${encodeURIComponent(TOKEN)}&id=${encodeURIComponent(id)}`);
10700:     const d = await r.json();
10701:     if (!d.ok) { toast('Error al eliminar', 'err'); return; }
```

### Referencia 51 — línea 10791

```javascript
10789:   try {
10790:     // Eliminar viejo → crear nuevo con los datos actualizados
10791:     const rDel = await fetch(`${APPS_SCRIPT_URL}?action=eliminarEvento&token=${encodeURIComponent(TOKEN)}&id=${encodeURIComponent(_eeeId)}`);
10792:     const dDel = await rDel.json();
10793:     if (!dDel.ok) { toast('Error al actualizar (eliminar): ' + (dDel.error||''), 'err'); return; }
```

### Referencia 52 — línea 10795

```javascript
10793:     if (!dDel.ok) { toast('Error al actualizar (eliminar): ' + (dDel.error||''), 'err'); return; }
10794: 
10795:     const rCre = await fetch(`${APPS_SCRIPT_URL}?action=crearEvento&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(data))}`);
10796:     const dCre = await rCre.json();
10797:     if (!dCre.ok) { toast('Error al actualizar (crear): ' + (dCre.error||''), 'err'); return; }
```

### Referencia 53 — línea 10810

```javascript
10808:   if (!confirm('¿Eliminar este evento?')) return;
10809:   try {
10810:     const r = await fetch(`${APPS_SCRIPT_URL}?action=eliminarEvento&token=${encodeURIComponent(TOKEN)}&id=${encodeURIComponent(_eeeId)}`);
10811:     const d = await r.json();
10812:     if (!d.ok) { toast('Error al eliminar', 'err'); return; }
```

### Referencia 54 — línea 10877

```javascript
10875: 
10876:   try {
10877:     const r = await fetch(`${APPS_SCRIPT_URL}?action=adminBook&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(data))}`);
10878:     const d = await r.json();
10879:     if (!d.ok) { toast('Error: ' + (d.error||''), 'err'); return; }
```

### Referencia 55 — línea 10891

```javascript
10889:         const nextData = { ...data, date: toDateStr(baseDate) };
10890:         try {
10891:           const rr = await fetch(`${APPS_SCRIPT_URL}?action=adminBook&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(nextData))}`);
10892:           const dd = await rr.json();
10893:           if (dd.ok) created++; else errors++;
```

### Referencia 56 — línea 11325

```javascript
11323:     for (const oldNombre of oldNombres) {
11324:       const data = encodeURIComponent(JSON.stringify({oldNombre, newNombre, telefono, email}));
11325:       const r = await fetch(`${APPS_SCRIPT_URL}?action=editPatient&token=${encodeURIComponent(TOKEN)}&data=${data}`);
11326:       const d = await r.json();
11327:       if (d.ok) totalActualizado += d.updated || 0;
```

### Referencia 57 — línea 11347

```javascript
11345:     // Borrar todos los nombres que usó este paciente
11346:     for (const nombre of p.nombres) {
11347:       await fetch(`${APPS_SCRIPT_URL}?action=deletePatient&token=${encodeURIComponent(TOKEN)}&nombre=${encodeURIComponent(nombre)}`);
11348:     }
11349:     allData.citas = allData.citas.filter(c => !p.nombres.includes(c.nombre));
```

### Referencia 58 — línea 11597

```javascript
11595:     for (const oldNombre of oldNombres) {
11596:       const data = encodeURIComponent(JSON.stringify({oldNombre, newNombre, telefono, email}));
11597:       const r = await fetch(`${APPS_SCRIPT_URL}?action=editPatient&token=${encodeURIComponent(TOKEN)}&data=${data}`);
11598:       const d = await r.json();
11599:       if (d.ok) totalActualizado += d.updated || 0;
```

### Referencia 59 — línea 11619

```javascript
11617:   try {
11618:     for (const nombre of p.nombres) {
11619:       const r = await fetch(`${APPS_SCRIPT_URL}?action=deletePatient&token=${encodeURIComponent(TOKEN)}&nombre=${encodeURIComponent(nombre)}`);
11620:       const d = await r.json();
11621:       if (!d.ok) { toast('Error al eliminar: ' + (d.error || 'intenta de nuevo'), 'err'); return; }
```

### Referencia 60 — línea 11670

```javascript
11668: 
11669:   try {
11670:     const r = await fetch(`${APPS_SCRIPT_URL}?action=adminBook&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(data))}`);
11671:     const d = await r.json();
11672:     if (d.ok) {
```

### Referencia 61 — línea 11681

```javascript
11679:       if (esReferido) {
11680:         try {
11681:           const rg = await fetch(`${APPS_SCRIPT_URL}?action=generarCodigo&token=${encodeURIComponent(TOKEN)}&tipo=REF`);
11682:           const dg = await rg.json();
11683:           if (dg.ok) {
```

### Referencia 62 — línea 11685

```javascript
11683:           if (dg.ok) {
11684:             const codData = { codigo: dg.codigo, tipo: 'REF', paciente: nombre, telefono, referidoPor };
11685:             await fetch(`${APPS_SCRIPT_URL}?action=registrarCodigo&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(codData))}`);
11686:             // Mostrar banner con el código
11687:             document.getElementById('dbCodigoValor').textContent = dg.codigo;
```

### Referencia 63 — línea 11767

```javascript
11765:   try {
11766:     const from = toDateStr(days[0]), to = toDateStr(days[6]);
11767:     const r = await fetch(`${APPS_SCRIPT_URL}?action=getCalEvents&token=${encodeURIComponent(TOKEN)}&from=${from}&to=${to}`);
11768:     const d = await r.json();
11769:     if (d.ok) _calGCevents = d.events || [];
```

### Referencia 64 — línea 12173

```javascript
12171:   cont.innerHTML = '<div class="loading-wrap"><div class="spinner"></div> Consultando base de datos...</div>';
12172:   try {
12173:     const r = await fetch(`${APPS_SCRIPT_URL}?action=getReminders&token=${encodeURIComponent(TOKEN)}`);
12174:     const d = await r.json();
12175:     if (!d.ok) { cont.innerHTML = '<div class="empty"><p>Error al cargar: ' + (d.error||'') + '</p></div>'; return; }
```

### Referencia 65 — línea 12288

```javascript
12286:   btn.textContent = 'Enviando...'; btn.disabled = true;
12287:   try {
12288:     const r = await fetch(`${APPS_SCRIPT_URL}?action=sendReminders&token=${encodeURIComponent(TOKEN)}`);
12289:     const d = await r.json();
12290:     if (d.ok) {
```

### Referencia 66 — línea 12673

```javascript
12671:   btn.textContent = 'Limpiando...'; btn.disabled = true;
12672:   try {
12673:     const r = await fetch(`${APPS_SCRIPT_URL}?action=cleanCitasSinHora&token=${encodeURIComponent(TOKEN)}`);
12674:     const d = await r.json();
12675:     if (d.ok) {
```

### Referencia 67 — línea 12758

```javascript
12756:   const nombre = decodeURIComponent(encNombre);
12757:   try {
12758:     const r = await fetch(`${APPS_SCRIPT_URL}?action=sendReminders&token=${encodeURIComponent(TOKEN)}`);
12759:     const d = await r.json();
12760:     if (d.ok) {
```

### Referencia 68 — línea 12902

```javascript
12900:   for (const dup of dups) {
12901:     try {
12902:       const r = await fetch(`${APPS_SCRIPT_URL}?action=cancelBooking&token=${encodeURIComponent(TOKEN)}&id=${encodeURIComponent(dup.id)}`).then(x => x.json());
12903:       if (r.ok) {
12904:         const cita = allData.citas.find(c => c.id === dup.id);
```

### Referencia 69 — línea 15962

```javascript
15960:   [btn, btn2].forEach(b => { if (b) { b.disabled = true; b.textContent = '⏳ Cargando...'; } });
15961:   try {
15962:     const r = await fetch(`${APPS_SCRIPT_URL}?action=getEncuestaStats&token=${encodeURIComponent(TOKEN)}`);
15963:     const d = await r.json();
15964:     if (d.ok) {
```

### Referencia 70 — línea 18484

```javascript
18482:   if (!confirm(`¿Marcar ${codigo} como Usado?\nEsto cambiará el estado en Google Sheets.`)) return;
18483:   try {
18484:     const r = await fetch(`${APPS_SCRIPT_URL}?action=actualizarCodigo&token=${encodeURIComponent(TOKEN)}&codigo=${encodeURIComponent(codigo)}&estado=Usado`);
18485:     const d = await r.json();
18486:     if (!d.ok) { toast('Error al actualizar el estado', 'err'); return; }
```

### Referencia 71 — línea 18514

```javascript
18512:       codigoRef:   codigoRef
18513:     };
18514:     const r = await fetch(`${APPS_SCRIPT_URL}?action=registrarCodigo&token=${encodeURIComponent(TOKEN)}&data=${encodeURIComponent(JSON.stringify(codData))}`);
18515:     const d = await r.json();
18516:     if (!d.ok) { toast('Error al generar bono', 'err'); return; }
```

### Referencia 72 — línea 18676

```javascript
18674:   if (!nombre) { card.style.display = 'none'; return; }
18675: 
18676:   const url = APPS_SCRIPT_URL + '?action=passportEnsure&token=' + encodeURIComponent(TOKEN)
18677:     + '&nombre=' + encodeURIComponent(nombre)
18678:     + '&telefono=' + encodeURIComponent(_pasTelefono || '');
```

### Referencia 73 — línea 18824

```javascript
18822: 
18823:   try {
18824:     const url = APPS_SCRIPT_URL + '?action=passportSaveProgress&token=' + encodeURIComponent(TOKEN)
18825:       + '&id=' + encodeURIComponent(_pasCurrent.id)
18826:       + '&passport=' + encodeURIComponent(JSON.stringify(passport))
```

### Referencia 74 — línea 18855

```javascript
18853:     // muestre exactamente la misma fuente de datos que la vista pública.
18854:     const nombre = document.getElementById('pasNombreInput').value.trim();
18855:     const verifyUrl = APPS_SCRIPT_URL + '?action=passportEnsure&token=' + encodeURIComponent(TOKEN)
18856:       + '&nombre=' + encodeURIComponent(nombre)
18857:       + '&telefono=' + encodeURIComponent(_pasTelefono || '')
```

### Referencia 75 — línea 18887

```javascript
18885:   if (!_pasCurrent || !_pasCurrent.id) return;
18886:   if (!confirm('¿Regenerar el enlace? El anterior dejará de funcionar.')) return;
18887:   const data = await fetch(APPS_SCRIPT_URL + '?action=passportRegenerateToken&token=' + encodeURIComponent(TOKEN) + '&id=' + encodeURIComponent(_pasCurrent.id)).then(r => r.json());
18888:   if (!data.ok) { toast(data.error || 'No se pudo regenerar', 'error'); return; }
18889:   _pasCurrent = data.passport;
```

### Referencia 76 — línea 18897

```javascript
18895:   if (!_pasCurrent || !_pasCurrent.id) return;
18896:   if (!confirm('¿Desactivar este pasaporte?')) return;
18897:   const data = await fetch(APPS_SCRIPT_URL + '?action=passportDeactivate&token=' + encodeURIComponent(TOKEN) + '&id=' + encodeURIComponent(_pasCurrent.id)).then(r => r.json());
18898:   if (!data.ok) { toast(data.error || 'No se pudo desactivar', 'error'); return; }
18899:   _pasCurrent = data.passport || _pasCurrent;
```

### Referencia 77 — línea 18907

```javascript
18905:   if (!_pasCurrent || !_pasCurrent.id) return;
18906:   if (!confirm('¿Reactivar este pasaporte?')) return;
18907:   const data = await fetch(APPS_SCRIPT_URL + '?action=passportReactivate&token=' + encodeURIComponent(TOKEN) + '&id=' + encodeURIComponent(_pasCurrent.id)).then(r => r.json());
18908:   if (!data.ok) { toast(data.error || 'No se pudo reactivar', 'error'); return; }
18909:   _pasCurrent = data.passport || _pasCurrent;
```

### Referencia 78 — línea 19467

```javascript
19465: 
19466:   try {
19467:     const url = `${APPS_SCRIPT_URL}?action=availability&date=${encodeURIComponent(fecha)}&service=${encodeURIComponent(servicio||'')}`;
19468:     const res  = await fetch(url).then(r => r.json());
19469:     if (!res.ok) throw new Error(res.error || 'Error');
```

### Referencia 79 — línea 19977

```javascript
19975:   if (btn) { btn.disabled = true; btn.style.opacity = '0.6'; }
19976:   try {
19977:     const r = await fetch(`${APPS_SCRIPT_URL}?action=getInactivos&token=${encodeURIComponent(TOKEN)}`);
19978:     const d = await r.json();
19979:     if (!d.ok) { panel.innerHTML = `<div style="color:var(--err,#ef4444);padding:20px">Error: ${d.error||'Sin respuesta del servidor'}</div>`; return; }
```
