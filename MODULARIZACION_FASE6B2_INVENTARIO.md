# Inventario de edición y estados de citas — Fase 6B2

- Funciones totales detectadas: 302
- Candidatas detectadas: 89

## Resumen de candidatas

- `kvRemove` — línea 6177
- `esc` — línea 6219 — acciones: teamData, saveProfessional, resetProfessionalPassword, toggleProfessional, deleteProfessional, markPayablePaid, repairRescheduledDuplicate, getKPIHistory, document
- `loadAutomationCenter` — línea 7671 — acciones: automationStatus, automationQueue
- `saveAutomationSettings` — línea 7693 — acciones: automationSave
- `_updateMobBadge` — línea 7807
- `esCobrada` — línea 7924
- `confirmarPago` — línea 7946 — acciones: updatePago
- `checkUpcomingAlerts` — línea 7965
- `exportarAgendaDia` — línea 7993
- `switchScheduleMode` — línea 8068
- `_updateSubmitLabel` — línea 8079
- `updateTimeHelp` — línea 8112
- `validateBusinessSchedule` — línea 8161
- `_addMultiDate` — línea 8174
- `_removeMultiDate` — línea 8189
- `_updateRecPreview` — línea 8229
- `confirmarReagendar` — línea 8268 — acciones: editBooking
- `chipState` — línea 8346
- `getInfoSesion` — línea 8390
- `waLink` — línea 8405
- `reload` — línea 8510 — acciones: adminData
- `esCobrada_UNUSED` — línea 8533
- `citasReales` — línea 8538
- `smartIsActiveAppointment` — línea 8583
- `appointmentHealth` — línea 8591
- `smartBriefingData` — línea 8612
- `patientInsightHtml` — línea 8769
- `initDashboard` — línea 8802
- `updateBadge` — línea 9066
- `resumenDiaWA` — línea 9074
- `guardarNotaAdmin` — línea 9111 — acciones: editBooking
- `changeStatus` — línea 9127 — acciones: updateStatus
- `verDetalle` — línea 9151
- `cancelarCita` — línea 9231
- `confirmarCancelacion` — línea 9247 — acciones: cancelBooking
- `getCancelMotivos` — línea 9277
- `saveCancelMotivo` — línea 9281
- `getCancelMotivo` — línea 9287
- `esCancelExcluida` — línea 9292
- `marcarErrorMio` — línea 9296
- `editarCita` — línea 9309
- `toggleDescuentoEdit` — línea 9344
- `calcDescuentoEdit` — línea 9349
- `calcDescuentoMontoEdit` — línea 9359
- `_showEditDescResult` — línea 9368
- `quitarDescuentoEdit` — línea 9373
- `getPrecioFinalEdit` — línea 9378
- `guardarEdicion` — línea 9388 — acciones: editBooking
- `renderBloqueos` — línea 9428
- `doUnblock` — línea 9464 — acciones: unblock
- `_addToServiceChips` — línea 9511
- `_removeServiceChip` — línea 9525
- `toggleSegundaPersona` — línea 9564
- `_updateDuoTime` — línea 9596
- `updateSesionesInfo` — línea 9722
- `sesionBadge` — línea 9730
- `submitEvento` — línea 9911 — acciones: crearEvento
- `eliminarEvento` — línea 9960 — acciones: eliminarEvento
- `cerrarModalEditarEvento` — línea 9984
- `abrirEditarEvento` — línea 9988
- `eeeGuardar` — línea 10027 — acciones: eliminarEvento, crearEvento
- `eeeEliminar` — línea 10071 — acciones: eliminarEvento
- `searchPatient` — línea 10097
- `renderChangeLog` — línea 10211
- `clearChangeLog` — línea 10252
- `renderPacientes` — línea 10261
- `_renderHistorial` — línea 10349
- `exportarHistorialPaciente` — línea 10370 — acciones: editPatient, deletePatient, adminBook, generarCodigo, registrarCodigo
- `cargarRecordatorios` — línea 11168 — acciones: getReminders
- `toggleSegFiltro` — línea 11318
- `renderSeguimiento` — línea 11382
- `_segCardReadap` — línea 11573
- `limpiarCitasSinHora` — línea 11659 — acciones: cleanCitasSinHora
- `exportarSeguimientoCSV` — línea 11684 — acciones: sendReminders, cancelBooking, getEncuestaStats, actualizarCodigo, registrarCodigo
- `_comisSemanasLlenas` — línea 17903
- `_comisReactivaciones` — línea 17921
- `_comisReventas` — línea 17950
- `_comisVentasCruzadas` — línea 17967
- `renderComisiones` — línea 18020
- `_msgCard` — línea 18399
- `abrirNuevoMensaje` — línea 18428
- `editarMensaje` — línea 18438
- `guardarMensaje` — línea 18449
- `eliminarMensaje` — línea 18468
- `gEditarToggle` — línea 18489
- `renderRecuperaciones` — línea 18608
- `eliminarRecuperacion` — línea 18750
- `cargarInactivos` — línea 18776 — acciones: getInactivos
- `_recInactivoCard` — línea 18903
...
function _updateMobBadge() {
  const n = (allData.citas || []).filter(c => c.estado === 'Pendiente').length;
  const b = document.getElementById('mobBadgeAgenda');
  if (!b) return;
...
function checkUpcomingAlerts() {
  ...
  const banner   = document.getElementById('upcomingAlert');
  if (!banner) return;

  const candidates = allData.citas
    .filter(c => c.estado !== 'Cancelada' && normDate(c.fecha) === todayStr && c.hora)
    .map(c => {
  ...
  if (!candidates.length) { banner.classList.remove('show'); return; }
  const next = candidates[0];
  const mins = Math.round(next.diff);
  document.getElementById('upcomingAlertTxt').innerHTML =
    `Próxima cita: <strong>${next.nombre}</strong> — ${next.servicio}`;
  document.getElementById('upcomingAlertMins').textContent =
    mins < 60 ? `en ${mins} min (${next.hora})` : `en ${Math.round(mins/60)}h ${mins%60}min (${next.hora})`;
...
```javascript
function exportarAgendaDia() {
  const todayStr = today();
  const citas = allData.citas
    .filter(c => normDate(c.fecha) === todayStr && c.estado !== 'Cancelada')
    .sort((a, b) => a.hora.localeCompare(b.hora));
  if (!citas.length) { toast('No hay citas hoy para exportar', 'err'); return; }

  const fecha = new Date().toLocaleDateString('es-CO', { weekday:'long', day:'numeric', month:'long', year:'numeric' });
  let txt = `AGENDA — ${fecha.toUpperCase()}\n${'='.repeat(52)}\n\n`;
  citas.forEach((c, i) => {
    txt += `${i+1}. ${c.hora}  ${c.nombre}\n`;
    txt += `   Servicio:  ${c.servicio}\n`;
    txt += `   Modalidad: ${c.modalidad}\n`;
    txt += `   Valor:     ${c.precio}${isPagada(c.id)?' ✓ PAGADA':' (pago pendiente)'}\n`;
    txt += `   Estado:    ${c.estado}\n`;
    if (c.telefono)  txt += `   Tel:       ${c.telefono}\n`;
    if (c.direccion) txt += `   Dirección: ${c.direccion}\n`;
    if (c.notas)     txt += `   Notas:     ${c.notas}\n`;
    txt += '\n';
  });
  txt += `Total: ${citas.length} cita(s)  |  Cobro pendiente: ${citas.filter(c=>!isPagada(c.id)).length}\n`;

  const blob = new Blob([txt], { type: 'text/plain;charset=utf-8' });
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement('a');
  a.href = url; a.download = `agenda-${todayStr}.txt`; a.click();
...
function chipState(st) {
  if (st === 'Confirmada') return '<span class="chip chip-ok">Confirmada</span>';
  if (st === 'Pendiente')  return '<span class="chip chip-warn">Pendiente</span>';
  if (st === 'Atendida')   return '<span class="chip chip-info">Atendida</span>';
  if (st === 'Cancelada')  return '<span class="chip chip-err">Cancelada</span>';
  if (st === 'No asistió') return '<span class="chip chip-no">No asistió</span>';
  if (st === 'Pendiente de pago') return '<span class="chip chip-warn">Pendiente de pago</span>';
  if (st === 'Comprobante recibido') return '<span class="chip chip-info">Comprobante recibido</span>';
  if (st === 'Pago verificado') return '<span class="chip chip-ok">Pago verificado</span>';
  if (st === 'Autorizada para atender') return '<span class="chip chip-ok">Autorizada para atender</span>';
  if (st === 'Sesión atendida') return '<span class="chip chip-info">Sesión atendida</span>';
  if (st === 'Reprogramada') return '<span class="chip chip-warn">Reprogramada</span>';
  if (st === 'Cancelada a tiempo') return '<span class="chip chip-err">Cancelada a tiempo</span>';
  if (st === 'Cancelación tardía') return '<span class="chip chip-err">Cancelación tardía</span>';
  if (st === 'Cortesía autorizada') return '<span class="chip chip-ok">Cortesía autorizada</span>';
  if (st === 'Reembolsada') return '<span class="chip chip-warn">Reembolsada</span>';
  return '<span class="chip chip-warn">' + (st||'—') + '</span>';
}
```
...
```javascript
function citasReales() {
  const seen = new Set();
  const result = allData.citas.filter(c => {
    if (c.estado === 'Cancelada' || c.estado === 'Registro' || esRegistroServ(c.servicio)) return false;
    if (!c.hora || isMidnightTime(c.hora)) return false;
  ...
  result.forEach(c => {
    if (!c.canal) {
      c.canal = (c.servicio || '').toLowerCase().includes('corporativ') ? 'Corporativo' : 'Directo';
    }
  });
  return result;
}
...
### smartIsActiveAppointment

```javascript
function smartIsActiveAppointment(c) {
  return c && c.estado !== 'Cancelada' && c.estado !== 'Registro' && !esRegistroServ(c.servicio);
}
```
...
```javascript
function appointmentHealth(c) {
  const f = normDate(c.fecha);
  const hoy = today();
  if (c.estado === 'Cancelada') return { rowClass:'agenda-alert', badge:'Cancelada', tone:'danger' };
  if (c.estado === 'No asistió') return { rowClass:'agenda-alert', badge:'No asistió', tone:'danger' };
  if (smartIsPaid(c) || c.estado === 'Atendida' || c.estado === 'Sesión atendida') return { rowClass:'agenda-ok', badge:smartIsPaid(c) ? 'Pagada' : 'Atendida', tone:'ok' };
  if (f && f < ADMIN_OPERATIONS_START_DATE) return { rowClass:'', badge:'Histórico', tone:'info' };
  if (c.estado === 'Pendiente') return { rowClass:'agenda-warn', badge:'Confirmar', tone:'warn' };
  if (f < hoy && !smartIsPaid(c)) return { rowClass:'agenda-warn', badge:'Cobro pendiente', tone:'warn' };
  if (f >= hoy && !smartIsPaid(c)) return { rowClass:'agenda-info', badge:'Programada', tone:'info' };
  return { rowClass:'', badge:'Revisar', tone:'info' };
}
```
...
```javascript
function smartBriefingData() {
  const hoy = today();
  const wk = smartWeekBounds();
  const citas = citasReales().filter(smartIsActiveAppointment).filter(c => isOperationalDate(c.fecha));
  const citasHoy = citas.filter(c => normDate(c.fecha) === hoy);
  const pendientesConfirmar = citas.filter(c => normDate(c.fecha) >= hoy && c.estado === 'Pendiente');
  const cobrosVencidos = citas.filter(c => normDate(c.fecha) < hoy && !smartIsPaid(c));
  const porCobrarSemana = citas.filter(c => {
    const f = normDate(c.fecha);
    return f >= hoy && f >= wk.startStr && f <= wk.endStr && !smartIsPaid(c);
  });
  const semanaCitas = citas.filter(c => {
    const f = normDate(c.fecha);
    return f >= wk.startStr && f <= wk.endStr;
  });
  const pacientes = new Map();
  citas.forEach(c => {
    const key = smartPatientKey(c);
    if (!key) return;
    const bucket = pacientes.get(key) || { nombre:c.nombre, citas:[] };
    bucket.citas.push(c);
    pacientes.set(key, bucket);
  });
  const pacientesSinProxima = [...pacientes.values()].filter(p => {
    const hist = p.citas.filter(c => normDate(c.fecha) <= hoy && c.estado !== 'Cancelada').sort((a,b)=>(normDate(b.fecha)+b.hora).localeCompare(normDate(a.fecha)+a.hora));
    const fut = p.citas.some(c => normDate(c.fecha) > hoy && c.estado !== 'Cancelada');
    if (!hist.length || fut) return false;
    const dias = Math.floor((smartDate(hoy) - smartDate(hist[0].fecha)) / 86400000);
    return dias >= 30;
  });
  const totalVencido = cobrosVencidos.reduce((s,c)=>s+parsePrecio(c.precio),0);
  const totalSemana = porCobrarSemana.reduce((s,c)=>s+parsePrecio(c.precio),0);
  const pctSesiones = META_SESIONES_SEMANA ? Math.round((semanaCitas.length / META_SESIONES_SEMANA) * 100) : 0;
  const score = Math.max(0, Math.min(100,
    100
    - Math.min(35, cobrosVencidos.length * 8)
    - Math.min(25, pendientesConfirmar.length * 5)
    - Math.min(20, pacientesSinProxima.length * 2)
    + (pctSesiones >= 100 ? 8 : 0)
  ));
  return { citasHoy, pendientesConfirmar, cobrosVencidos, porCobrarSemana, pacientesSinProxima, totalVencido, totalSemana, semanaCitas, pctSesiones, score };
}
```
...
```javascript
function patientInsightHtml(c) {
  const key = smartPatientKey(c);
  const lista = allData.citas
    .filter(x => smartPatientKey(x) === key || (x.nombre || '').toLowerCase().trim() === (c.nombre || '').toLowerCase().trim())
    .filter(smartIsActiveAppointment)
    .sort((a,b)=>(normDate(b.fecha)+b.hora).localeCompare(normDate(a.fecha)+a.hora));
  const hoy = today();
  const futuras = lista.filter(x => normDate(x.fecha) > hoy && x.id !== c.id && x.estado !== 'Cancelada').sort((a,b)=>(normDate(a.fecha)+a.hora).localeCompare(normDate(b.fecha)+b.hora));
  const realizadas = lista.filter(x => normDate(x.fecha) <= hoy && x.estado !== 'Cancelada');
  const total = lista.reduce((s,x)=>s+parsePrecio(x.precio),0);
  const pendiente = lista.filter(x => !smartIsPaid(x)).reduce((s,x)=>s+parsePrecio(x.precio),0);
  let log = [];
  try { log = JSON.parse(kvGet('adminChangeLog') || '[]'); } catch { log = []; }
  const last = log.find(e => (e.detalle || '').toLowerCase().includes((c.nombre || '').toLowerCase().split(' ')[0] || '__'));
  return `<div class="patient-insight">
    <div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;flex-wrap:wrap">
      <div>
        <div style="font-family:var(--font-m);font-size:.72rem;color:var(--muted);text-transform:uppercase;letter-spacing:.08em">Resumen inteligente del paciente</div>
        <div style="font-size:.86rem;color:var(--muted);margin-top:3px">${futuras[0] ? `Próxima: ${esc(fmtDate(futuras[0].fecha))} ${esc(futuras[0].hora)} · ${esc(futuras[0].servicio)}` : 'No tiene próxima cita adicional registrada.'}</div>
      </div>
      <button type="button" class="btn btn-ghost btn-sm" onclick="verHistorial('${encodeURIComponent(c.nombre)}')">Ver historial completo</button>
    </div>
    <div class="patient-insight-grid">
      <div class="patient-insight-card"><span>Citas totales</span><strong>${lista.length}</strong></div>
      <div class="patient-insight-card"><span>Realizadas</span><strong>${realizadas.length}</strong></div>
      <div class="patient-insight-card"><span>Valor histórico</span><strong style="font-size:1rem">${smartPeso(total)}</strong></div>
      <div class="patient-insight-card"><span>Por marcar pago</span><strong style="font-size:1rem;color:${pendiente ? '#d97706' : 'var(--primary)'}">${smartPeso(pendiente)}</strong></div>
    </div>
    ${last ? `<div style="margin-top:9px;font-size:.76rem;color:var(--muted)">Último movimiento local: <strong>${esc(last.accion)}</strong> · ${esc(last.ts)}</div>` : ''}
  </div>`;
}
```
...
function initDashboard() {
  ...
  const hoy    = citas.filter(c => normDate(c.fecha) === todayStr).length;
  const semana = citas.filter(c => { if (!c.hora) return false; const [cy,cm,cd]=normDate(c.fecha).split('-'); const d=new Date(+cy,+cm-1,+cd); return d>=startW && d<=endW; }).length;
  const mes    = citas.filter(c => { const [cy,cm]=normDate(c.fecha).split('-'); return +cm===m && +cy===y; }).length;
  const pacs   = new Set(citas.map(c => c.telefono || c.email || (c.nombre||'').toLowerCase().trim())).size;
  ...
  document.getElementById('stPacientes').textContent = pacs;

  // Cobrado = citas del mes ya ocurridas (fecha <= hoy) + eventos del mes ya ocurridos
  const citasMesAll = citas.filter(c => { const [cy,cm]=normDate(c.fecha).split('-'); return +cm===m && +cy===y; });
  const cobradoCitas = citasMesAll.filter(c => normDate(c.fecha) <= todayStr).reduce((s,c) => s + parsePrecio(c.precio), 0);
  const cobradoEventosMes = (allData.eventos || [])
    .filter(e => { const [cy,cm]=normDate(e.fecha).split('-'); return +cm===m && +cy===y && normDate(e.fecha) <= todayStr; })
    .reduce((s,e) => s + parsePrecio(e.cobro), 0);
  const cobrado = cobradoCitas + cobradoEventosMes;
  document.getElementById('stCobrado').textContent = cobrado > 0 ? '$'+cobrado.toLocaleString('es-CO') : '—';

  const cobradoSemana = citas
    .filter(c => { const f=normDate(c.fecha); const [cy,cm,cd]=f.split('-'); const d=new Date(+cy,+cm-1,+cd); return d>=startW && d<=endW && f<=todayStr; })
    .reduce((s,c) => s + parsePrecio(c.precio), 0)
    + (allData.eventos || [])
      .filter(e => { const f=normDate(e.fecha); const [cy,cm,cd]=f.split('-'); const d=new Date(+cy,+cm-1,+cd); return d>=startW && d<=endW && f<=todayStr; })
      .reduce((s,e) => s + parsePrecio(e.cobro), 0);
  document.getElementById('stCobradoSemana').textContent = cobradoSemana > 0 ? '$'+cobradoSemana.toLocaleString('es-CO') : '—';

  const cobradoHoy = citas
    .filter(c => normDate(c.fecha) === todayStr)
    .reduce((s,c) => s + parsePrecio(c.precio), 0)
    + (allData.eventos || [])
      .filter(e => normDate(e.fecha) === todayStr)
      .reduce((s,e) => s + parsePrecio(e.cobro), 0);
  document.getElementById('stCobradoHoy').textContent = cobradoHoy > 0 ? '$'+cobradoHoy.toLocaleString('es-CO') : '—';

  // Por cobrar este mes = citas FUTURAS del mes + eventos futuros del mes con cobro
  const porCobrarEventos = (allData.eventos || [])
    .filter(e => { const [cy,cm]=normDate(e.fecha).split('-'); return +cm===m && +cy===y && normDate(e.fecha) > todayStr; })
    .reduce((s,e) => s + parsePrecio(e.cobro), 0);
  const porCobrar = citasMesAll
    .filter(c => normDate(c.fecha) > todayStr)
    .reduce((s,c) => s + parsePrecio(c.precio), 0) + porCobrarEventos;
  document.getElementById('stPorCobrar').textContent = porCobrar > 0 ? '$'+porCobrar.toLocaleString('es-CO') : '—';

  // Por cobrar esta semana = citas FUTURAS dentro de la semana actual
  const porCobrarSemanaEventos = (allData.eventos || [])
    .filter(e => { const f=normDate(e.fecha); const [cy,cm,cd]=f.split('-'); const d=new Date(+cy,+cm-1,+cd); return d>=startW && d<=endW && f>todayStr; })
    .reduce((s,e) => s + parsePrecio(e.cobro), 0);
  const porCobrarSemana = citas
    .filter(c => { const f=normDate(c.fecha); const [cy,cm,cd]=f.split('-'); const d=new Date(+cy,+cm-1,+cd); return d>=startW && d<=endW && f>todayStr; })
    .reduce((s,c) => s + parsePrecio(c.precio), 0) + porCobrarSemanaEventos;
  document.getElementById('stPorCobrarSemana').textContent = porCobrarSemana > 0 ? '$'+porCobrarSemana.toLocaleString('es-CO') : '—';

  // Meta mensual
  actualizarMetaBarra(cobrado);
  ...
  initQuickNotes();
  renderSmartBriefing();
  renderSmartCobrosCenter();
  setTimeout(_animateDashStats, 80);
  setTimeout(_updateMobBadge, 200);
  // Revisar alerta cada minuto
  if (window._alertInterval) clearInterval(window._alertInterval);
  window._alertInterval = setInterval(checkUpcomingAlerts, 60000);

  // Timeline hoy
  const citasHoy = citas.filter(c => normDate(c.fecha) === todayStr).sort((a,b) => a.hora.localeCompare(b.hora));
  const tlHoy = document.getElementById('tlHoy');
  if (citasHoy.length === 0) {
    tlHoy.innerHTML = '<div class="empty"><p>No hay citas hoy</p></div>';
  } else {
    tlHoy.innerHTML = citasHoy.map(c => {
      const h = appointmentHealth(c);
      return `
      <div class="tl-item ${h.rowClass}">
        <div class="tl-time">${esc(c.hora)}</div>
        <div class="tl-body">
          <div class="tl-name">${esc(c.nombre)} ${chipState(c.estado)} <span class="health-badge ${h.tone}">${esc(h.badge)}</span></div>
          <div class="tl-serv">${esc(c.servicio)} · ${esc(c.modalidad)} · ${esc(c.precio)}</div>
          <div class="tl-actions">
            ${waBtn(c.telefono,c.nombre,c.fecha,c.hora,c.servicio,'WhatsApp',c.precio,c.modalidad)}
            <button type="button" class="btn btn-ghost btn-sm" onclick="verDetalle('${esc(c.id)}')">Ver</button>
            ${pagoBadge(c.id)}
            <button type="button" class="btn btn-ghost btn-sm" onclick="changeStatus('${esc(c.id)}','Atendida')">✓ Atendida</button>
          </div>
        </div>
      </div>`;
    }).join('');
  ...
  // Proximas citas (7 dias excluyendo hoy)
  const tm = new Date(); tm.setDate(tm.getDate()+1);
  const in7 = new Date(); in7.setDate(in7.getDate()+7);
  const proximas = citas
    .filter(c => { const [y2,m2,d2]=normDate(c.fecha).split('-'); const dt=new Date(+y2,+m2-1,+d2); return dt>=tm && dt<=in7; })
    .sort((a,b) => (normDate(a.fecha)+a.hora).localeCompare(normDate(b.fecha)+b.hora))
    .slice(0, 8);

  const tlP = document.getElementById('tlProximas');
  if (proximas.length === 0) {
    tlP.innerHTML = '<div class="empty"><p>No hay citas en los próximos 7 días</p></div>';
  } else {
    tlP.innerHTML = proximas.map(c => {
      const waUrl = waLink(c.telefono,c.nombre,c.fecha,c.hora,c.servicio,c.precio,c.modalidad);
      const h = appointmentHealth(c);
      return `
      <div class="tl-item ${h.rowClass}">
        <div class="tl-time" style="min-width:80px">${esc(fmtDate(c.fecha))}<br><small style="color:var(--muted)">${esc(c.hora)}</small></div>
        <div class="tl-body">
          <div class="tl-name">${esc(c.nombre)} <span class="health-badge ${h.tone}">${esc(h.badge)}</span></div>
          <div class="tl-serv">${esc(c.servicio)} · ${esc(c.modalidad)}</div>
          <div class="tl-actions">
            ${waUrl ? `<a href="${waUrl}" target="_blank" class="btn btn-wa btn-sm">WhatsApp</a>` : ''}
            <button type="button" class="btn btn-ghost btn-sm" onclick="verDetalle('${esc(c.id)}')">Ver</button>
          </div>
        </div>
      </div>`;
  ...
  // Banner tareas pendientes en dashboard
  renderTareas();
  // Alerta semana floja en dashboard
  _checkAlertaSemanFloja(citasReales());
  renderSmartBriefing();
  renderSmartCobrosCenter();
}
```
...
```javascript
function resumenDiaWA() {
  const todayStr = today();
  const hoy = citasReales()
    .filter(c => normDate(c.fecha) === todayStr)
    .sort((a,b) => (a.hora||'').localeCompare(b.hora||''));
  const eventosHoy = (allData.eventos||[])
    .filter(e => normDate(e.fecha) === todayStr)
    .sort((a,b) => (a.horaInicio||'').localeCompare(b.horaInicio||''));
  if (!hoy.length && !eventosHoy.length) { toast('No hay citas ni eventos hoy', 'err'); return; }
  const fecha = new Date().toLocaleDateString('es-CO',{weekday:'long',day:'numeric',month:'long'});
  const lines = ['📋 *Agenda del ' + fecha + ':*', ''];
  hoy.forEach((c,i) => {
    lines.push((i+1) + '. ' + c.hora + ' — *' + c.nombre + '*');
    lines.push('   ' + c.servicio + ' · ' + c.modalidad + ' · ' + c.precio);
    lines.push('   Estado: ' + c.estado);
    lines.push('');
  });
  if (eventosHoy.length) {
    lines.push('⚡ *Eventos del día:*', '');
    eventosHoy.forEach(e => {
      lines.push('• ' + (e.horaInicio||'') + '–' + (e.horaFin||'') + ' — *' + (e.titulo||'') + '*');
      lines.push('   ' + (e.tipo||'') + ' · ' + (e.cobro||''));
      lines.push('');
    });
  }
  lines.push('Total: ' + hoy.length + ' cita(s)' + (eventosHoy.length ? ' + ' + eventosHoy.length + ' evento(s)' : ''));
  window.open('https://wa.me/573136467945?text=' + encodeURIComponent(lines.join('\n')), '_blank');
}
```
...
```javascript
function _addToServiceChips(val) {
  if (!val || val === '__planes__') return;
  if (_selectedServices.includes(val)) {
    toast('Ese servicio ya fue agregado', 'warn'); return;
  }
  _selectedServices.push(val);
  _renderServiceChips();
  document.getElementById('ncService').value = _selectedServices.join(' + ');
  autoFillPrice();
  updateSesionesInfo();
  checkDomicilioWarn();
  _updateDuoTime();
...
```javascript
function _removeServiceChip(idx) {
...
### toggleSegundaPersona

```javascript
function toggleSegundaPersona() {
  _duoActive = !_duoActive;
  const panel = document.getElementById('ncDuoPanel');
  const btn   = document.getElementById('ncDuoBtn');
  panel.style.display = _duoActive ? 'block' : 'none';
  btn.style.background   = _duoActive ? 'rgba(139,92,246,.14)' : 'rgba(139,92,246,.06)';
  btn.style.borderStyle  = _duoActive ? 'solid' : 'dashed';
  if (_duoActive) _updateDuoTime();
}
```
...
```javascript
function _updateDuoTime(...args) {
  const module = window.PanelAppointmentCreate;
  if (!module || typeof module._updateDuoTime !== 'function') {
    throw new Error('El módulo de creación de citas no está disponible: _updateDuoTime');
  }
  return module._updateDuoTime(...args);
}
```
...
```javascript
function updateSesionesInfo(...args) {
  const module = window.PanelAppointmentCreate;
  if (!module || typeof module.updateSesionesInfo !== 'function') {
    throw new Error('El módulo de creación de citas no está disponible: updateSesionesInfo');
  }
  return module.updateSesionesInfo(...args);
}
```
...
```javascript
function sesionBadge(nombre, servicio) {
  const n = sesionesPorPaquete[servicio];
  if (!n) return '';
  const norm   = (nombre||'').toLowerCase().trim();
  const hechas = allData.citas.filter(c =>
    (c.nombre||'').toLowerCase().trim() === norm &&
    c.servicio === servicio && c.estado !== 'Cancelada'
  ).length;
  const rest  = Math.max(0, n - hechas);
  const color = rest === 0 ? '#dc2626' : rest === 1 ? '#d97706' : '#059669';
  return `<br><span style="font-size:.72rem;color:${color};font-weight:700">📊 ${hechas}/${n} sesiones · ${rest} restantes</span>`;
}
```
